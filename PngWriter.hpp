...
private:
  ...
  unsigned char zeroOneTo256(double r) {
      r = r > 0.999999 ? 0.999999 : r;
      r = r < 0 ? 0 : r;
      return (unsigned char) floor(r * 256);
  }
 ...
 public:
  ...
  void resize(const int newWidth,const int newHeight) {
    ...
    unsigned char (*oldBuffer)[3] = buffer;
    ...
    // copy old data
    for (int i = 0; i < nx && i < oldNx; ++i) {
        for (int j = 0; j < ny && j < oldNy; ++j) {
            for (int k = 0; k < 3; ++k) {
                buffer[(ny-j-1)*nx+i][k] = 
                    oldBuffer[(oldNy-j-1)*oldNx+i][k];
            }
        }
    }
  ...
  void set(int i,int j,double r,double g,double b) {
      set(i, j, zeroOneTo256(r), zeroOneTo256(g), zeroOneTo256(b));
  }
